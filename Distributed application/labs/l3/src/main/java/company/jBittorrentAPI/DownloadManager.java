...
public class DownloadManager implements DTListener, PeerUpdateListener,
    ...
    public void closeTempFiles() {
        for (RandomAccessFile output_file : this.output_files)
            try {
                output_file.close();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public synchronized void savePiece(int piece) {
        ...
        for (Iterator it = this.pieceList[piece].getFileAndOffset().keySet().iterator(); it.hasNext(); ) {
            try {
                ...
                this.output_files[file].write(data, data.length - remainingData, Math.min(remaining, remainingData));
        ...
        synchronized (this) {
            synchronized (this.isComplete) {
                ...
                for (int i = 0; i < this.nbOfFiles; i++) {
                    try {
                        new File(saveAs).mkdirs();
                        FileOutputStream fos = new FileOutputStream(saveAs + this.torrent.name.get(i));
                        ...
                        ioe.printStackTrace();
                        System.err.println("Error when saving the file " + this.torrent.name.get(i));
                    }
                }
            }
        }
    ...
    private synchronized void unchokePeers() {
        synchronized (this.task) {
            ...
            if (!this.isComplete())
                l.sort(new DLRateComparator());
            else
                l.sort(new ULRateComparator());
        ...
        for (Iterator it = this.pieceList[piece].getFileAndOffset().keySet().
            ...
            try {
                ...
                this.output_files[file].read(data,
                        data.length - remainingData, Math.min(remaining, remainingData));
                remainingData -= remaining;
            } catch (IOException ioe) {
                System.err.println(ioe.getMessage());
            }
