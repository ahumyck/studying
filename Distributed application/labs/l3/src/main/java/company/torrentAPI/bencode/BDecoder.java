...
package company.torrentAPI.bencode;

import company.jBittorrentAPI.Constants;
...
public class BDecoder {
    ...
    public static Map<Object, Object> decode(byte[] data) throws IOException {
        return (new BDecoder().decodeByteArray(data));
    }

    public static Map<Object, Object> decode(BufferedInputStream is) throws IOException {
        return (new BDecoder().decodeStream(is));
    }

    public static Map<Object, Object> decode(File file) throws IOException {
        return new BDecoder().decodeStream(new BufferedInputStream(FileUtils.openInputStream(file)));
    }

    @SneakyThrows
    public static Map<Object, Object> decode(HttpGetRequest request) {
        return new BDecoder().decodeStream(request.sendRequestAndGetResponseAsBytes());
    }
    ...
    public Map<Object, Object> decodeByteArray(byte[] data) throws IOException {
        return (decode(new ByteArrayInputStream(data)));
    }

    public Map<Object, Object> decodeStream(BufferedInputStream data) throws IOException {
    ...
    private Map<Object, Object> decode(ByteArrayInputStream data) throws IOException {
        Object res = decodeInputStream(data, 0);
        if (res == null) {
            throw (new IOException("BDecoder: zero length file"));
        } else if (!(res instanceof Map)) {
